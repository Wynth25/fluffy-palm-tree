...
class t:
	...
	bright_black = '\033[90m'  # Often shows as dark gray
	bright_red = '\033[91m'
...
def visualize(grid, reqdb=0):
		...
		if a == 1:
				print(f"{t.grid}{grid[0][0]} {grid[0][1]} {grid[0][2]} {grid[0][3]}{t.end}")
				print(f"{t.grid}{grid[1][0]} {grid[1][1]} {grid[1][2]} {grid[1][3]}{t.end}")
				print(f"{t.grid}{grid[2][0]} {grid[2][1]} {grid[2][2]} {grid[2][3]}{t.end}")
				print(f"{t.grid}{grid[3][0]} {grid[3][1]} {grid[3][2]} {grid[3][3]}{t.end}")
...
for label, data_list in wasd_lists.items():
		...
		# data_list --> ['wwww', 'wwwa'...]
		if folders_debug == 1:
			print(f"{t.folder}Opening folder:{t.end} {label}")
		if any_debug > 0:
			print("")

		for combo in data_list:
				if combo_debug == 1:
					print(f"{t.black}Combo:{t.end} {combo}")
				pr = 3
				pc = 3
...
#				 wwasdwwawasasd
				...
				if flat_list_debug == 1:
					print(f"{t.blue}Flat list:{t.end} {flat_list_num}\n")
