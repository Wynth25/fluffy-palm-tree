...
for label, data_list in wasd_lists.items():
	...
	for combo in data_list:
		...
		#print(flat_list_num)
		a_wasd_result.append( (len(combo), combo, flat_list_num) )
...
for label, data_list in wasd_lists.items():
	...
	for combo in data_list:
		...
		flat_list_num = [int(i, 16) if isinstance(i, str) else i for i in flat_list_num]
		#print(flat_list_num)
		b_wasd_result.append( (len(combo), combo, flat_list_num) )
...
while idx_a < len(a_wasd_result) and idx_b < len(b_wasd_result):
	moves_a, combo_a, grid_a = a_wasd_result[idx_a]
	moves_b, combo_b, grid_b = b_wasd_result[idx_b]
	if grid_a == grid_b:
		printd("\nsuccess!!!")
		printd(f"Moves A: {moves_a}. Combo: {combo_a}")
		printd(f"Moves B: {moves_b}. Combo: {combo_b}")
		if moves_a != moves_b:
			input()
		idx_a += 1
		idx_b += 1
