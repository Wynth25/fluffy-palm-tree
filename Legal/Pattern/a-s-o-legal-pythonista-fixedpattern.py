...
# 1. Define the starting state outside the loops
initial_grid = [
    [1, 2, 3, 4],
    [5, 6, 7, 8],
    [9, "a", "b", "c"],
...
# 2. Setup our memory bank. 
# Key: combo string (""). Value: (grid_list, pr, pc)
grid_states = {"": (initial_grid, 3, 3)} 
...
for label, data_list in wasd_lists.items():
    ...
    for combo in data_list:
        ...
        # 3. Look up the exact grid state of the PREVIOUS sequence
        prev_combo = combo[:-1]  # 'wwas' becomes 'wwa'
        prev_grid, pr, pc = grid_states[prev_combo]

        # 4. CRITICAL: We must "Deep Copy" the previous grid so we don't accidentally
        # overwrite the saved parent state while calculating the child state.
        grid = [row[:] for row in prev_grid]
        
        # 5. We ONLY need to execute the very last character of the combo!
        last_move = combo[-1]
        ...
        # 6. Save this new state into memory for the next layer of folders to use
        grid_states[combo] = (grid, pr, pc)

        # Flatten the grid and append it to your final results exactly as before
...
visual_grids_99 = [tuple(x if x in (1,2,3,4,5,6,7,8) else 99 for x in grid) for grid in a_wasd_result]

visual_grids_99.sort()

visual_grids_ = [tuple(x if x != 99 else " " for x in grid) for grid in visual_grids_99]
