...
def gv_seq(current_seq, target_len, current_list):
		...
		if len(current_seq) == target_len:
				...
				return
		for char in allowed:
				if (char == current_seq[-1]) or current_seq[-1] == allowed[(allowed.index(char) + 2) % 4]:
						continue
...
def main():
	...
	print(f"Sequence lenght: {upper_range}")
	...
	for length in range(1, upper_range+1):
			for start_char in allowed:
					...
					printd(f"Created folder: {list_name}")
	...
	#				 wwasdwwawasasd
					...
					for item_combo in combo:
							...
							if item_combo == 'w':
									grid[(pr + 0) % 3][pc] = grid[(pr + 1) % 3][pc]
									grid[(pr + 1) % 3][pc] = grid[(pr + 2) % 3][pc]
									grid[(pr - 1) % 3][pc] = "_"
									visualize(grid, 2)
									pr = (pr - 1) % 3
							...
							if item_combo == 'a':
									grid[pr][(pc + 0) % 3] = grid[pr][(pc + 1) % 3]
									grid[pr][(pc + 1) % 3] = grid[pr][(pc + 2) % 3]
									grid[pr][(pc - 1) % 3] = "_"
									visualize(grid, 2)
									pc = (pc - 1) % 3
					...
					flat_list_num = [9 if x == '_' else x for x in flat_list]
					flat_list_num = [int(i, 9) if isinstance(i, str) else i for i in flat_list_num]
					printd(flat_list_num)
...
for upper_range in range (1, run_times + 1):
	main()
